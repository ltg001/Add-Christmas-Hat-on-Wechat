...
def Del_GroupMsg(msg):
    ...
    if msg.type == PICTURE: #如果接受到图片，就自动回复同样的图片
        ...
        #savePath = os.path.join('pic', msg.file_name)
        savePath = os.path.join('pic', msg.file_name)
        msg.get_file(savePath)
        #outPath = os.path.join('out', msg.file_name)
        outPath = "out\\" + msg.file_name
        print(outPath, savePath)
        img = cv2.imread(savePath)
        out_image = add_hat_func(img, hat_img)
        cv2.imwrite(outPath, out_image)
        msg.reply_image(outPath)
